...
public class FlightData extends FlightDataTable {
	 ...
	 */
	/**
	public void readParquetVariants()throws IOException {
		...
		try {
			...
			ReadSupport<FlightDataRecord> readSupport = new ReadSupport<FlightDataRecord>();
		
			try (ParquetReader<SimpleGroup> reader = ParquetReader.builder(ReadSupport, HadoopInputFile.fromPath(parquetFilePath, conf)).build()) {
	            SimpleGroup record;
	            while ((record = reader.read()) != null) {
	                // Example: Handle null values for a specific field
	                String fieldValue = record.getString("altitude", 0);
	                if (fieldValue == null) {
	                    fieldValue = "default_value"; // Replace null with a default value
	                }
